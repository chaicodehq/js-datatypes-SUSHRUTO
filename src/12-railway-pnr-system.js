...
export function processRailwayPNR(pnrData) {
  ...
  if (
    !pnrData.train ||
    typeof pnrData.train !== "object" ||
    typeof pnrData.train.number !== "string" ||
    typeof pnrData.train.name !== "string" ||
    typeof pnrData.train.from !== "string" ||
    typeof pnrData.train.to !== "string"
  ) {
    return null;
  }
  ...
  const trainInfo =
    `Train: ${number} - ${name} | ${from} → ${to} | Class: ${pnrData.classBooked}`;
  ...
  const passengers = pnrData.passengers.map(p => {
    ...
    if (current === "CAN") {
      ...
      statusLabel = "WAITING"; // safest fallback
    }
  ...
  const chartPrepared =
    ...
    nonCancelled.every(p => p.isConfirmed);
